...
用 `open-wepig-cli` 完成发现、看参数、调用三步。脚本已经封装底层访问细节；本 skill 只说明怎么用，以及什么时候读取 references。
...
| endpoint 0 命中、keyword 难选、返回 schema 字段语义不清 | 对应 service 的 reference |
| 新增 service | 在 `references/` 新增同名领域文档，并在本表追加 |
